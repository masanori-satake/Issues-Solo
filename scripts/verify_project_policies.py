...
def verify_no_external_libraries():
            ...
            for script in manifest.get("content_scripts", []):
                for js in script.get("js", []):
                    if re.match(r"^(?:https?:)?//", js):
                        print(
                            f"Error: External script found in manifest.json (content_scripts): {js}"
                        )
                        passed = False
            ...
            for resource in manifest.get("web_accessible_resources", []):
                ...
                for res in resources:
                    if re.match(r"^(?:https?:)?//", res):
                        print(
                            f"Error: External resource found in manifest.json (web_accessible_resources): {res}"
                        )
                        passed = False
    ...
    external_script_src_pattern = re.compile(
        r'<script[^>]+src=["\']((?:https?:)?//[^"\']+)["\']', re.IGNORECASE
    )
    external_link_href_pattern = re.compile(
        r'<link[^>]+href=["\']((?:https?:)?//[^"\']+)["\']', re.IGNORECASE
    )
            ...
            if file.endswith(".html"):
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        for src in scripts:
                            print(
                                f"Error: External script source found in {file_path}: {src}"
                            )
                            passed = False
