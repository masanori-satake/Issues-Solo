...
describe("IssueRenderer", () => {
  ...
  test("should render issue items", async () => {
    ...
    expect(items[0].querySelector(".issue-title").textContent).toBe(
      "Test Issue",
    );
  });
  ...
  test("should sort issues by lastAccessed desc by default", async () => {
    const issues = [
      ...
      {
        url: "https://h1.atlassian.net/browse/K2",
        issueKey: "K2",
        title: "T2",
        lastAccessed: 2000,
      },
    ...
    db.getAllIssues.mockResolvedValue(issues);
    db.getSettings.mockResolvedValue([
      { id: "1", name: "Jira", url: "h1.atlassian.net", visible: true },
    ]);

    await renderer.render();

    const keys = Array.from(listElement.querySelectorAll(".issue-key")).map(
      (el) => el.textContent,
    );
  ...
  test("should group issues by host", async () => {
    const issues = [
      ...
      {
        url: "https://h2.atlassian.net/browse/K2",
        issueKey: "K2",
        title: "T2",
      },
    ];
  ...
  test("should handle host with path", async () => {
    const issues = [
      {
        url: "https://jira.com/context/browse/K1",
        issueKey: "K1",
        title: "T1",
      },
  ...
  test("should handle collapsed host", async () => {
    ...
    db.getSettings.mockResolvedValue([
      ...
      },
      {
        id: "2",
        name: "H2",
        url: "h2.atlassian.net",
        visible: true,
        isCollapsed: false,
      }, // アコーディオン表示をトリガーするため2つ目のホストを追加
    ]);
  ...
  test("should group by project key", async () => {
    const issues = [
      {
        url: "https://h1.atlassian.net/browse/PROJA-1",
        issueKey: "PROJA-1",
        title: "T1",
      },
      {
        url: "https://h1.atlassian.net/browse/PROJB-1",
        issueKey: "PROJB-1",
        title: "T2",
      },
    ];
    ...
    const projectHeaders = listElement.querySelectorAll(
